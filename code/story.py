...
def pick():
    ...
    traps=[a for a in d if a.get('raw_pnl',0)>=2000 and a.get('copier_pnl',0)<0 and key(a) not in used]
    traps.sort(key=lambda a:-(a['raw_pnl']+abs(a['copier_pnl'])))  # biggest combined impact
    if traps: return _story(traps[0],'trap')
    ...
    if gems: return _story(gems[0],'gem')
    return None
def _story(a,kind):
    _mark(a['addr'])
    return dict(kind=kind,addr=a['addr'],raw_pnl=a['raw_pnl'],copier_pnl=a['copier_pnl'],
